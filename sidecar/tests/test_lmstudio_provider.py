...
async def test_factory_lmstudio_resolution():
    settings = {"ai_lmstudio_host": "http://my-lm-studio:1234/v1"}
...
async def test_lmstudio_model_listing():
    settings = {"ai_lmstudio_host": "http://localhost:1234/v1"}
    provider = AIProviderFactory.get_provider("lmstudio", settings=settings)

    mock_resp_data = {"data": [{"id": "publisher/model-1"}, {"id": "publisher/model-2"}]}
