...
def test_anomaly_detection_persistence(app):
    ...
    for h in range(1, 25):
        ...
        for _ in range(10):
            cursor.execute("INSERT INTO logs (workspace_id, cluster_id, timestamp) VALUES (?, ?, ?)", ("ws1", "c1", ts))
    
    # 2. Setup current anomaly data (last 5 minutes)
    # 100 logs in last 5 minutes (scaled to hourly rate = 1200, which is >> 10)
    ts_now = now.strftime("%Y-%m-%d %H:%M:%S")
    for _ in range(100):
        cursor.execute("INSERT INTO logs (workspace_id, cluster_id, timestamp) VALUES (?, ?, ?)", ("ws1", "c1", ts_now))
