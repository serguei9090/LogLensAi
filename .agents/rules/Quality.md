...
- **Functions/Classes (The Contract)**: 
  - **JS/TS**: Every exported function, interface, or class MUST have a full `JSDoc` block. Include a `@link` or `Ref:` to the relevant architecture doc.
  - **Python**: Every public function or class MUST have a full `Google-style Docstring`. Include a `Ref:` to the relevant architecture doc.
  - Both must explicitly define: **Purpose**, **Context** (Architecture Ref), **Args**, and **Returns**.
...
- **Complex Logic**: If a block of logic is highly algorithmic, write a multi-line comment above it mapping out the steps in plain English.
...
- **The Formal TODO Syntax**: 
  - `// TODO(ID): [WHAT] Need to add debounce. [WHY] API limits hit. [EXPECTATION] Smooth typing experience. [CONTEXT] See docs/track/specs/ID.md`
