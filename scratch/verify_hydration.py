...
def test_hydration():
    ...
    source_id = "test_src"
    from models import IngestLogsRequest, IngestLogEntry
    
    logs_to_ingest = [
        IngestLogEntry(workspace_id=workspace_id, source_id=source_id, raw_text="First log line", timestamp="2024-01-01 10:00:01", level="INFO", message="First log line"),
        IngestLogEntry(workspace_id=workspace_id, source_id=source_id, raw_text="Second log line with error", timestamp="2024-01-01 10:00:02", level="ERROR", message="Second log line with error"),
        IngestLogEntry(workspace_id=workspace_id, source_id=source_id, raw_text="Third log line for debug", timestamp="2024-01-01 10:00:03", level="DEBUG", message="Third log line for debug"),
    ]
    ...
    for log in result['logs']:
        print(f"ID: {log['id']}, Line ID: {log['line_id']}, Level: {log['level']}, Message: '{log['message']}'")
