...
| `sidecar/src/ai/` | `AIRunner` | Unified AI orchestration layer (Graph, Runner, Providers). |
| `sidecar/src/anomalies.py` | `AnomalyDetector` | Statistical analyzer for identifying outliers and novel log patterns. |
| `sidecar/src/ingestion.py` | `IngestionServer` | High-throughput buffer for incoming log batches before persistence. |
| `sidecar/src/metadata_extractor.py`| `extract_log_metadata` | Regex-based parser for structured fields (JSON, KV pairs). |
| `sidecar/src/mcp_server.py` | `MCPServer` | Integration with Model Context Protocol for tool-calling capabilities. |
| `sidecar/src/models.py` | `RPCModels` | Pydantic validation schemas for all JSON-RPC request/response pairs. |
| `sidecar/src/query_parser.py` | `LLQLParser` | LogLens Query Language (LLQL) compiler for advanced DuckDB filtering. |
