...
## 🔌 AI Providers (sidecar/src/ai/)

LogLensAi supports a hybrid model ecosystem, allowing users to switch between local and cloud providers.

### 1. `ai_studio.py` (Google AI Studio)
- **Primary Model**: Gemini 1.5 Pro / Flash.
- **Capabilities**: Native tool-calling, long-context (1M+ tokens), multimodal support.
- **Protocol**: Uses the `google-generativeai` SDK.

### 2. `ollama.py` (Local Inference)
- **Primary Model**: Gemma 4 (e4b), Llama 3.
- **Capabilities**: Optimized for local privacy, streaming thinking mode (Gemma).
- **Protocol**: Standard Ollama API on port 11434.

### 3. `openai_compatible.py` (Universal)
- **Primary Model**: Any OpenAI-compatible API (Mistral, vLLM, Groq).
- **Capabilities**: Fallback for any standard REST API.
- **Protocol**: Standard OpenAI Chat Completions schema.

### 4. `gemini_cli.py` (Local Bridge)
- **Purpose**: Direct bridge to the Google Gemini CLI for low-latency agentic execution.
- **Usage**: Typically used for "Surgical Edit" workflows and heavy code generation.

---

## 🏗️ Intelligence Core Modules

| File | Responsibility |
|---|---|
| `context.py` | `SmartContextManager`: Prunes log data and chat history to fit context windows. |
| `thinking_parser.py` | `ThinkingStreamParser`: Advanced state machine for identifying split thinking tokens. |
| `runner.py` | `HybridRunner`: The main orchestrator that connects LangGraph to the sidecar API. |
