...
import { X, Cpu, Zap, Check, Clock, Save, Settings2, Sparkles, ChevronDown, Plus } from "lucide-react";
...
function TimezoneSelect({ value, onChange }: { readonly value: number; readonly onChange: (v: number) => void }) {
  ...
  const updatePosition = () => {
    ...
    setPanelStyle({ position: "fixed", top: rect.bottom + 4, left: rect.left, width: Math.max(rect.width, 148), zIndex: 9999 });
  };
  ...
  return (
    <>
      <button
        ref={triggerRef}
        type="button"
        onClick={() => { updatePosition(); setOpen(o => !o); }}
        className={cn(
          "flex items-center gap-2 h-8 px-3 rounded-md text-[11px] font-mono font-semibold transition-all",
          "bg-[#0D0F0E] border text-white",
          open ? "border-violet-500 ring-1 ring-violet-500/40 text-violet-400" : "border-white/10 hover:border-violet-400/40 hover:text-violet-300"
        )}
      >
          ...
          {TIMEZONE_OPTIONS.map((opt) => (
            <button
              ...
              className={cn(
                ...
                opt.value === value ? "bg-violet-500/20 text-violet-400 font-bold" : "text-white/70 hover:bg-white/5 hover:text-white"
              )}
            >
              {opt.label}
            </button>
...
function StrategyCard({ icon, name, description, onClick }: {
...
export function OrchestratorHub({
    ...
    if (editingFusionId) {
      ...
      callSidecar<{ sources: FusionSourceConfig[] }>({
        ...
        const merged = availableSources.map(src => {
          ...
          return existing ?? { source_id: src.path, enabled: false, tz_offset: 0, custom_format: null, parser_config: null };
        });
        setConfigs(merged);
      }).catch(() => {
  ...
  const toggleSource = (sourceId: string) =>
    setConfigs(prev => prev.map(c => c.source_id === sourceId ? { ...c, enabled: !c.enabled } : c));

  const updateOffset = (sourceId: string, offset: number) =>
    setConfigs(prev => prev.map(c => c.source_id === sourceId ? { ...c, tz_offset: offset } : c));

  const handleParserSaved = (sourceId: string, configJson: string) => {
    setConfigs(prev => prev.map(c => c.source_id === sourceId ? { ...c, parser_config: configJson } : c));
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-[10px] text-text-muted uppercase tracking-widest font-medium">
                {view === "picker" ? "Choose a Strategy" : editingFusionId ? "Edit Fusion" : "New Fusion"}
              </p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {view === "picker" && (
            <div className="p-5 space-y-4">
              <p className="text-[11px] text-text-muted uppercase tracking-widest font-bold">Available Strategies</p>
              <StrategyCard
          ...
          {view === "fusion-form" && (
            <div className="p-5 space-y-6">
              ...
              <div className="space-y-2">
                <label htmlFor="fusion-name" className="text-[10px] font-bold text-text-muted uppercase tracking-widest flex items-center gap-2">
              ...
              {/* Source List */}
              <div className="space-y-2">
                <p className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Log Sources</p>
                {isLoadingConfig ? (
                  <div className="py-8 text-center text-text-muted/40 animate-pulse text-sm">Loading config...</div>
                ) : configs.length === 0 ? (
                  <div className="py-8 text-center text-text-muted/40 text-sm italic">No log sources in workspace yet.</div>
                ) : (
                  <div className="space-y-2">
                    {configs.map((config) => {
                      const sourceInfo = availableSources.find((s) => s.path === config.source_id);
                      const label = sourceInfo?.name ?? config.source_id.split("/").pop() ?? config.source_id;
                      ...
                      return (
                        <div
                          ...
                          className={cn(
                            ...
                            config.enabled ? "bg-violet-500/5 border-violet-500/20" : "bg-black/20 border-border/20 opacity-60"
                          )}
                        >
                          {/* Toggle */}
                          <button
                            ...
                            className={cn(
                              ...
                              config.enabled ? "bg-violet-500 text-white" : "bg-white/5 border border-white/10"
                            )}
                          >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="block text-[10px] text-text-muted truncate lowercase font-mono">{config.source_id}</span>
                          </div>

                          {/* Timezone */}
                          <TimezoneSelect value={config.tz_offset} onChange={(tz) => updateOffset(config.source_id, tz)} />
              ...
              <div className="p-3 rounded-xl bg-violet-500/5 border border-violet-500/10 text-[11px] text-text-muted leading-relaxed">
                <span className="font-bold text-violet-400">Sync Strategy:</span> Interleaved Timestamp Alignment.
                Use <span className="text-text-secondary font-bold">Sync Drift</span> to offset logs from different timezones.
              </div>
            </div>
          )}
        </div>
        ...
        {view === "fusion-form" && (
          <div className="p-5 border-t border-border/40 bg-white/5 shrink-0 space-y-3">
            <button
              ...
              disabled={isSaving || !fusionName.trim()}
              className={cn(
