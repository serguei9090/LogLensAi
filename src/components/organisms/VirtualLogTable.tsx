...
import { ArrowDown, ArrowUp, ArrowUpDown, ChevronDown, MessageSquarePlus, X } from "lucide-react";
...
export function VirtualLogTable({
      ...
      <>
        {parts.map((part, i) => {
          const match = highlights.find(
            (h) => h.term && h.term.toLowerCase() === part.toLowerCase(),
          );
  ...
  return (
    <div
      ref={parentRef}
      className="h-full w-full overflow-auto bg-bg-base border border-border rounded-xl relative select-text custom-scrollbar"
    >
      <div
        style={{
      ...
      >
        <table className="w-full text-left text-sm whitespace-nowrap table-fixed">
          <thead className="sticky top-0 bg-bg-surface border-b border-border z-10 text-text-muted text-[10px] font-bold uppercase tracking-widest h-10 select-none">
            <tr>
              <th
                className="w-[60px] px-3 py-1 cursor-pointer hover:text-text-primary transition-colors text-center"
                onClick={() => onSort("id")}
              >
              ...
              <th
                className="w-[180px] px-3 py-1 cursor-pointer hover:text-text-primary transition-colors"
              ...
              <th
                className="w-[90px] px-3 py-1 cursor-pointer hover:text-text-primary transition-colors"
              ...
              <th
                className="w-[110px] px-3 py-1 text-center cursor-pointer hover:text-text-primary transition-colors"
              ...
              <th
                className="w-[90px] px-3 py-1 text-center cursor-pointer hover:text-text-primary transition-colors"
        ...
        <div className="sticky bottom-0 left-0 w-full bg-bg-surface border-t border-border p-6 shadow-2xl z-20 animate-in slide-in-from-bottom duration-300 backdrop-blur-md bg-white/5 max-h-[500px] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-3">
              <label className="text-[10px] font-bold uppercase tracking-widest text-text-muted opacity-60">
                Raw Data
              </label>
