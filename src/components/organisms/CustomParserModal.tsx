...
import { X, Search, Terminal, Info, Wand2, Save, Trash2, Clock } from "lucide-react";
...
export function CustomParserModal({
  ...
  const [activeSelection, setActiveSelection] = useState<{ start: number; end: number; text: string; rect: DOMRect } | null>(null);
  const [mapping, setMapping] = useState<{ timestamp?: { start: number; end: number }; level?: { start: number; end: number } }>({});
  ...
  const renderLineWithHighlights = (line: string) => {
    ...
    return segments.map((s, i) => (
        <span 
            key={i} 
            className={cn(
                s.type === 'timestamp' && "bg-primary/30 text-primary border-b-2 border-primary px-0.5 rounded-t-sm",
                s.type === 'level' && "bg-cyan-500/30 text-cyan-400 border-b-2 border-cyan-500 px-0.5 rounded-t-sm"
            )}
        >
            {s.text}
        </span>
        ...
        <div className="max-w-5xl w-full my-auto mx-auto min-h-[520px] flex flex-col bg-[#0D1110] border border-border/60 rounded-2xl shadow-2xl overflow-hidden ring-1 ring-white/10">
          ...
          <div className="flex items-center justify-between p-6 border-b border-border/40 bg-white/5">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h2 className="text-lg font-bold text-text-primary tracking-tight">Parser Architect</h2>
                <div className="flex items-center gap-2 mt-0.5">
                   <p className="text-xs text-text-muted uppercase tracking-widest font-medium">Log Translation Engine</p>
                   <div className="h-1 w-1 rounded-full bg-white/20" />
                   <span className="text-[10px] text-primary font-mono">{mapping.timestamp ? "Sync Active" : "Unmapped"}</span>
                </div>
              </div>
            </div>
            <button onClick={onClose} className="p-2 hover:bg-white/10 rounded-full transition-colors text-text-muted hover:text-text-primary">
              <X className="size-5" />
            </button>
          </div>
               ...
               <div className="p-4 border-b border-border/20 flex items-center justify-between">
                  <span className="text-[10px] font-bold text-text-muted uppercase tracking-widest flex items-center gap-2">
                   ...
                   <Tooltip>
                     ...
                     <TooltipContent side="bottom" className="bg-[#111613] border-white/20 text-white p-2 text-[11px] max-w-[240px]">
                       Select a range to map it to a field.
                     </TooltipContent>
                   </Tooltip>
                </div>

               <div ref={sampleContainerRef} onMouseUp={handleMouseUp} className="flex-1 overflow-auto p-0 font-mono text-xs select-text custom-scrollbar bg-surface-base/20 divide-y divide-white/5">
                  {!isLoading && samples.length > 0 ? samples.map((line, idx) => (
                    <div key={idx} data-sample-line="true" className="group flex hover:bg-primary/5 transition-colors min-w-fit">
            ...
            <div className="flex-1 flex flex-col min-w-[320px] bg-surface-base/40 backdrop-blur-xl">
               <div className="p-6 space-y-6">
                  <div className="space-y-3">
                      <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest flex items-center justify-between">
                        <div className="flex items-center gap-2"><Wand2 className="size-3" /> Extraction Logic</div>
                        {regexPattern && <span className="text-[9px] text-primary/60 lowercase italic">Python Sync Standard</span>}
                     </label>
                         ...
                         <div className="absolute top-2 right-2 flex flex-col gap-2">
                            {activeSelection && (
                                <div className="flex flex-col gap-1 p-1 bg-primary/10 border border-primary/20 rounded-lg animate-in slide-in-from-right-2">
                                    <button 
                                      onClick={() => {
                                         setMapping(prev => ({ ...prev, timestamp: { start: activeSelection.start, end: activeSelection.end } }));
                                         setActiveSelection(null);
                                      }}
                                    ...
                                    <button 
                                      onClick={() => {
                                         setMapping(prev => ({ ...prev, level: { start: activeSelection.start, end: activeSelection.end } }));
                                         setActiveSelection(null);
                                      }}
                            ...
                            <Tooltip>
                              ...
                              <TooltipContent side="left" className="bg-[#111613] border-white/20 text-white p-3 text-[11px] max-w-[280px] space-y-2 text-wrap">
                                <p className="font-bold text-primary">Python Regex Protocol</p>
                                <p>LogLens uses Python-style named groups:</p>
                                <div className="space-y-1.5 font-mono text-[10px]">
                                  <code className="block bg-black/40 p-1.5 rounded border border-white/5 text-primary-hover">(?P&lt;timestamp&gt;...)</code>
                                  <code className="block bg-black/40 p-1.5 rounded border border-white/5 text-primary-hover">(?P&lt;level&gt;...)</code>
                                </div>
                              </TooltipContent>
                            </Tooltip>
                         </div>
                  ...
                  <div className="space-y-4">
                    <label className="text-[10px] font-bold text-text-muted uppercase tracking-widest">Extraction Preview</label>
                    <div className="bg-black/40 rounded-xl border border-white/5 divide-y divide-white/5 overflow-hidden text-wrap">
                       {previewResults.length > 0 ? previewResults.slice(0, 2).map((res, i) => (
                         <div key={i} className="p-3 flex items-center justify-between text-[11px]">
                            <span className="text-text-muted font-mono">{res.timestamp || "---"}</span>
                            <span className={cn(
                              "px-1.5 py-0.5 rounded text-[9px] font-bold uppercase",
                              res.level === "ERROR" ? "bg-red-500/20 text-red-400" : "bg-primary/20 text-primary"
                            )}>
                               {res.level}
                            </span>
                         </div>
                       )) : (
                         <div className="p-8 text-center text-text-muted/40 italic text-[11px]">Enter regex to see preview</div>
                       )}
                    </div>
                  </div>
               </div>
               ...
               <div className="px-6 pb-2">
                  <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 space-y-2">
                     ...
                     <ul className="text-[10px] text-text-secondary space-y-1.5 list-disc list-inside">
                        ...
                        <li>Check the <span className="font-bold underline">Preview</span> for live validation.</li>
                     </ul>
                  </div>
               </div>
               ...
               <div className="mt-auto p-6 border-t border-border/40 bg-white/5 space-y-4">
                  ...
                  <div className="flex gap-3">
                     <button onClick={() => { setRegexPattern(""); setMapping({}); }} className="p-2.5 rounded-xl border border-border/40 text-text-muted hover:text-red-400"><Trash2 className="size-5" /></button>
                     <button onClick={handleSave} className="flex-1 bg-primary text-black h-11 rounded-xl font-bold text-sm hover:bg-primary-hover shadow-lg">Save Definition</button>
                  </div>
               </div>
            </div>
          </div>
